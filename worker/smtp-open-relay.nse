...
function go(host, port)
  ...
  local tests = {
	...
	-- Utilisateur vers externe
    {
      from = string.format("%s@%s", from, domain),
      to = "pentest1@algosecure.fr"
    },
	-- Externe vers externe
    {
      from = "pentest1@algosecure.fr",
      to = "pentest2@algosecure.fr"
    },
  }
...
action = function(host, port)
  ...
  if not status then
    ...
    if #result > 0 then
      local final = {}
      table.insert(final,
        string.format("Server is an open relay (%i/3 tests)",
        (#result)))
